//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "MMUIViewController.h"

#import "EmoticonBackupOperateMgrExt.h"
#import "UIAlertViewDelegate.h"

@class MMGifView, NSData, NSString, UIImage, UIImageView;

@interface EmoticonPickViewController : MMUIViewController <EmoticonBackupOperateMgrExt, UIAlertViewDelegate>
{
    UIImage *m_image;
    NSData *m_imageData;
    UIImageView *m_emoticonBkgView;
    NSString *m_localMd5;
    MMGifView *_m_gifView;
    UIImageView *_m_imageView;
}

@property(retain, nonatomic) UIImageView *m_imageView; // @synthesize m_imageView=_m_imageView;
@property(retain, nonatomic) MMGifView *m_gifView; // @synthesize m_gifView=_m_gifView;
@property(retain, nonatomic) NSString *m_localMd5; // @synthesize m_localMd5;
@property(retain, nonatomic) UIImageView *m_emoticonBkgView; // @synthesize m_emoticonBkgView;
@property(retain, nonatomic) NSData *m_imageData; // @synthesize m_imageData;
@property(retain, nonatomic) UIImage *m_image; // @synthesize m_image;
- (void).cxx_destruct;
- (void)onAddBackupEmoticonFailed:(id)arg1 isOverLimit:(_Bool)arg2;
- (void)onAddBackupEmoticonOK:(id)arg1;
- (void)alertView:(id)arg1 clickedButtonAtIndex:(long long)arg2;
- (void)viewDidLoad;
- (void)Confirm;
- (void)Cancel;
- (void)showTipsAndQuit:(id)arg1;
- (void)initToolBar;
- (void)dealloc;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

